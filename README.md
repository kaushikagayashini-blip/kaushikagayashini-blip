<h1 align="center">Hi 👋, I'm Kaushi Ambegoda</h1>
<h3 align="center">Software Engineering Undergraduate | UI/UX Designer | Full Stack Developer in the Making</h3>

<br>

<p align="center">
  <a href="https://linkedin.com/in/kaushiambegoda" target="_blank">
    <img src="https://img.shields.io/badge/LINKEDIN-Kaushi%20Ambegoda-0077B5?style=for-the-badge&logo=linkedin&logoColor=white" />
  </a> <br>
  <a href="https://github.com/kaushikagayashini-blip" target="_blank">
    <img src="https://img.shields.io/badge/GITHUB-kaushikagayashini--blip-181717?style=for-the-badge&logo=github&logoColor=white" />
  </a> <br>
  <a href="mailto:kaushikagayashini@gmail.com" target="_blank">
    <img src="https://img.shields.io/badge/GMAIL-kaushikagayashini%40gmail.com-D14836?style=for-the-badge&logo=gmail&logoColor=white" />
  </a> <br>
  <p>
  <img src="https://img.shields.io/badge/📍_Location-Rathnapura,_Sri_Lanka-EA4335?style=for-the-badge" />
</p>

</p>

---

## 🚀 About Me

🎓 Undergraduate at **Cardiff Metropolitan University** (BSc Hons Software Engineering)

💼 Aspiring **Full Stack Developer**, with a strong interest in **UI/UX Design** and **QA & Software Testing**

🎯 Passionate about creating clean, user-centered digital experiences

🌱 Currently learning:
- Advanced Web Development
- Full Stack Development
- Database Management & Design
- QA & Software Testing

💡 Interested in:
- UI/UX Design
- Front-End Development
- Software Engineering
- Quality Assurance & Testing

---

## 🛠️ Tech Stack

**Languages**
<p>
  <img src="https://skillicons.dev/icons?i=html,css,js,java,php" />
</p>

**Database**
<p>
  <img src="https://skillicons.dev/icons?i=mysql" />
</p>

**Development Tools**
<p>
  <img src="https://skillicons.dev/icons?i=git,github,vscode" />
</p>

---

## 🎨 Skills

```
HTML/CSS          ██████████████████░░  90%
JavaScript        ████████████████░░░░  80%
PHP                ████████████████░░░░  80%
Java               ██████████████░░░░░░  70%
MySQL              ████████████████░░░░  80%
UI/UX Design       ██████████████████░░  90%
```
## 💡 Professional Skills

- 🧩 Software Design & Development
- 🔍 Expertise in OOP Concepts
- 🧠 Problem-Solving & Critical Thinking
- ✅ Quality Assurance (QA) & Testing
- 🔄 Software Development Life Cycle (SDLC)
- 🤝 Team Work & Collaboration
- 🗄️ Database Management & Design
- 😊 Positive Attitude & Communication Skills
---
## 💡 Languages
<p>
  <img src="https://img.shields.io/badge/English-4B8BBE?style=for-the-badge" /> <br>
  <img src="https://img.shields.io/badge/Sinhala-FFB703?style=for-the-badge" />
</p>

## 📁 Featured Projects

**🎬 MondyCinema — Online Cinema Booking & Management System**
Cinema reservation platform with customer booking, profile management, and an admin panel for movie listings and approvals.
`PHP` `HTML` `CSS` `JavaScript` `MySQL`

**🎓 University Management System — Web-Based Academic Platform**
Student registration, course selection, grading, and admin record management with role-based access control.
`HTML` `CSS` `PHP` `MySQL`

**👟 CityStyle Footwear Management System — OOP-Based Retail System**
GUI retail system automating inventory tracking, product search, account management, and return approvals.
`Java (OOP)` `GUI` `MySQL`

**📅 University Event Management System — Cross-Platform Event App**
Cross-platform app for event discovery, RSVP, and feedback with real-time backend syncing.
`Flutter` `Dart` `PHP` `MySQL`

**🚗 Ride Row — Campus Carpooling Platform**
Front-end development of a team-built carpooling platform with authentication, ride publishing/search, and real-time messaging.
`React` `JavaScript` `GitHub`

**♟️ ABC Chess Academy — Tournament Management System**
Desktop application managing academy workflow: authentication, player/tournament administration, and error handling.
`Java` `Text File Storage` `Apache`

---

## 🏆 Achievements

🏅 Microsoft Certified: Diploma in Computer Application (Sarvodaya Fusion)

🎓 Software Engineering Undergraduate

🚀 Building real-world software solutions across web, mobile, and desktop platforms

---

## 🌍 Let's Connect

I'm always interested in:

🔹 UI/UX Design Discussions<br>
🔹 Front-End & Full Stack Development<br>
🔹 Open Source Projects<br>
🔹 QA & Software Testing<br>
🔹 Creative Collaborations<br>

<p align="center"><i>⭐️ Thanks for visiting my profile!</i></p>
